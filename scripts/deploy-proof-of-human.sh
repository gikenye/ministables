...
SCOPE_SEED=${SCOPE_SEED:-"minilend-app"}
...
if [ -n "$CELOSCAN_API_KEY" ]; then
    ...
    # verificationConfig struct: {olderThan: 18, forbiddenCountries: ["NORTH_KOREA"], ofacEnabled: true}
    CONSTRUCTOR_ARGS=$(cast abi-encode "constructor(address,string,bool))" \
