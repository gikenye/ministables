...
import { ArrowLeft, Smartphone, AlertCircle, CheckCircle, Loader2, Info, X } from "lucide-react"
import {
  onrampService,
  SUPPORTED_COUNTRIES,
  formatPhoneNumber,
...
const BottomSheet = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      {/* Bottom Sheet */}
      <div className={`relative w-full ${maxHeight} bg-black rounded-t-3xl shadow-2xl transform transition-transform duration-300 ease-out animate-in slide-in-from-bottom`}>
        {children}
      </div>
    </div>
  );
...
const ActionButton = ({
  ...
  const baseClasses = "rounded-xl font-semibold transition-colors flex items-center justify-center gap-2";
  const sizeClasses = {
    default: "px-4 py-3 text-sm",
    lg: "px-6 py-4 text-base",
  };
  const variantClasses = {
    primary: "bg-cyan-400 text-black hover:bg-cyan-300 disabled:bg-gray-600 disabled:text-gray-400",
...
/**
 * Render a multi-step bottom-sheet modal to deposit a specified asset via mobile money.
 *
 * @param isOpen - Whether the modal is visible
 * @param onClose - Callback to close the modal
 * @param selectedAsset - The asset identifier used for onramp operations
 * @param assetSymbol - Human-readable symbol shown in the UI (e.g., "CELO", "USDC")
 * @param onSuccess - Optional callback invoked when a deposit is successfully initiated; receives the transaction code and amount
 * @returns A React element containing the multi-step mobile-money deposit UI (country/network selection, phone validation, amount entry, and payment status)
 */
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm">Choose your mobile money provider</p>
              </div>

              <div className="space-y-4">
                <div>
                  <Label className="text-gray-400 text-sm font-medium mb-2 block">Country</Label>
                  <Select
                ...
                <div>
                  <Label className="text-gray-400 text-sm font-medium mb-2 block">Mobile Network</Label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent className="bg-gray-800 border-gray-600">
                      {availableNetworks.map((network) => (
                        <SelectItem key={network} value={network} className="text-white hover:bg-gray-700">
                          {network}
                        </SelectItem>
                      ))}
                    </SelectContent>
