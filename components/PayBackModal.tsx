...
export function PayBackModal({ isOpen, onClose, onPayBack, tokenInfos, loading }: PayBackModalProps) {
    ...
    try {
      ...
      for (const tokenAddress of supportedStablecoins) {
        ...
        if (borrowAmount && borrowAmount !== "0") {
          const tokenInfo = tokenInfos[tokenAddress] || (await getTokenInfo(tokenAddress))

          // Estimate interest (simplified calculation - in real app you'd get this from contract)
          const principal = Number(formatAmount(borrowAmount, tokenInfo.decimals))
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md mx-auto bg-white border-0 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label className="text-sm font-medium text-gray-700 mb-3 block">Your Active Loans</Label>

            {loadingLoans ? (
              <div className="text-center py-4">
                <div className="animate-spin w-6 h-6 border-2 border-primary border-t-transparent rounded-full mx-auto"></div>
                <p className="text-sm text-gray-600 mt-2">Loading your loans...</p>
              </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center">
                        <span className="text-lg mr-2">{getTokenFlag(loan.symbol)}</span>
                        <div>
                          <span className="font-medium text-gray-900">{loan.symbol}</span>
                      ...
                      <div className="text-right">
                        <div className="font-semibold text-red-600">
                          {formatAmount(loan.totalOwed, loan.decimals)} {loan.symbol}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-gray-50 rounded p-2">
                        ...
                        <div className="font-medium">
                          {formatAmount(loan.principal, loan.decimals)} {loan.symbol}
                        </div>
                      </div>
                      <div className="bg-yellow-50 rounded p-2">
                        <div className="text-gray-600">Interest</div>
                        <div className="font-medium text-yellow-700">
                          {formatAmount(loan.estimatedInterest, loan.decimals)} {loan.symbol}
                        </div>
                      </div>
                    </div>

                    {selectedLoan?.token === loan.token && (
                      <div className="mt-2 p-2 bg-primary/10 rounded text-xs text-primary">✓ Selected for payment</div>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-6 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-500">You don't have any outstanding loans to pay back.</p>
              </div>
            )}
          </div>
            ...
            <>
              <div className="border-t pt-4">
                ...
                <div className="space-y-3">
                  <div>
                    <Label htmlFor="payback-token" className="text-sm font-medium text-gray-700">
                      Loan to Pay Back
                    </Label>
                    <Select
                      value={form.token}
                      ...
                      <SelectContent>
                        {activeLoans.map((loan) => (
                          <SelectItem key={loan.token} value={loan.token}>
                            <div className="flex items-center">
                              <span className="mr-2">{getTokenFlag(loan.symbol)}</span>
                              {loan.symbol} - {formatAmount(loan.totalOwed, loan.decimals)} owed
                            </div>
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  <div>
                    <Label htmlFor="payback-amount" className="text-sm font-medium text-gray-700">
                      Payment Amount
                    </Label>
                    <Input
                      id="payback-amount"
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <div className="flex items-start">
                        ...
                        <div className="text-sm">
                          <div className="font-medium text-blue-800 mb-1">Payment Breakdown</div>
                          <div className="text-blue-700 space-y-1">
                            <div>
                              Amount: {form.amount} {selectedLoan.symbol}
                            </div>
                            <div className="text-xs">
                              This will reduce your outstanding debt. Any overpayment will be refunded.
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  ...
                  disabled={loading || !form.token || !form.amount || Number(form.amount) <= 0}
